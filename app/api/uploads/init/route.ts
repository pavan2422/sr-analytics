...
export async function POST(req: Request) {
  // Debug warning for Serverless + No DB
  if (!process.env.DATABASE_URL && (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME)) {
    console.warn(
      'WARINING: Running in Serverless mode without DATABASE_URL. ' +
      'SQLite will use /tmp which is ephemeral and NOT shared between requests. ' +
      'Uploads > 10MB will likely FAIL with "Session not found". ' +
      'Please configure DATABASE_URL to a persistent database (Postgres/MySQL/etc).'
    );
  }
